...
function Add-ArtAsset {
    ...
    $prompt = "Create a production-ready 2D game asset for DungeonFit. Category: $Category. Role: $Role. Visual direction: $Look. Exact canvas: $($size.Width)x$($size.Height) px. Transparent background required: $($(if ($Transparent.HasValue) { $Transparent.Value } else { $defaults.Transparent })). Nine-slice/stretch-safe frame required: $($(if ($NineSlice.HasValue) { $NineSlice.Value } else { $defaults.NineSlice })). Style: dark moonlit fantasy RPG, purple and gold accents, crisp pixel-inspired rendering, mobile readability. Do not include baked text, watermarks, logos, signatures, UI labels, or extra margins unless the role requires a full background."
    $assets[$key] = [ordered]@{
        ...
        transparent = if ($Transparent.HasValue) { $Transparent.Value } else { $defaults.Transparent }
        nineSlice = if ($NineSlice.HasValue) { $NineSlice.Value } else { $defaults.NineSlice }
        exists = $size.Exists
        ...
        finalChecks = @(
            ...
            [ordered]@{ id = 'transparent_background'; label = 'Transparent background'; required = if ($(if ($Transparent.HasValue) { $Transparent.Value } else { $defaults.Transparent })) { 'Required' } else { 'Not required' } },
